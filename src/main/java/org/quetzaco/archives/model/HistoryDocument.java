package org.quetzaco.archives.model;

public class HistoryDocument {
    private String id;

  private String wjbh;
  private String djrq;
  private String sxh;

    private String qzh;

    private String dh;

    private String zrz;

    private String bgqx;

    private String mj;

    private String ztc;

    private String bz;

    private String nd;

    private String stfs;

    private String jh;

    private String ys;

    private String wh;

    private String tm;

    private String stms;

    private String lbbh;

    private String ngbm;

    private String wjxcsj;

    private String stflh;

    private String gdrq;

    private String ssbm;

    private String wjsx;

    private String lrr;

    private String wjlx;

    private String yjr;

    private String djh;

    private String sysFileCount;

    private String swlx;

    private String swsm;

    private String xcsj;

    private String fs;

    private String kh;

    private String ajh;

    private String sl;

    private String documentType;

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id == null ? null : id.trim();
    }

  public String getWjbh() {
    return wjbh;
  }

  public void setWjbh(String wjbh) {
    this.wjbh = wjbh == null ? null : wjbh.trim();
  }

  public String getDjrq() {
    return djrq;
  }

  public void setDjrq(String djrq) {
    this.djrq = djrq == null ? null : djrq.trim();
  }

  public String getSxh() {
    return sxh;
  }

  public void setSxh(String sxh) {
    this.sxh = sxh == null ? null : sxh.trim();
  }


    public String getQzh() {
        return qzh;
    }

    public void setQzh(String qzh) {
        this.qzh = qzh == null ? null : qzh.trim();
    }

    public String getDh() {
        return dh;
    }

    public void setDh(String dh) {
        this.dh = dh == null ? null : dh.trim();
    }

    public String getZrz() {
        return zrz;
    }

    public void setZrz(String zrz) {
        this.zrz = zrz == null ? null : zrz.trim();
    }

    public String getBgqx() {
        return bgqx;
    }

    public void setBgqx(String bgqx) {
        this.bgqx = bgqx == null ? null : bgqx.trim();
    }

    public String getMj() {
        return mj;
    }

    public void setMj(String mj) {
        this.mj = mj == null ? null : mj.trim();
    }

    public String getZtc() {
        return ztc;
    }

    public void setZtc(String ztc) {
        this.ztc = ztc == null ? null : ztc.trim();
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz == null ? null : bz.trim();
    }

    public String getNd() {
        return nd;
    }

    public void setNd(String nd) {
        this.nd = nd == null ? null : nd.trim();
    }

    public String getStfs() {
        return stfs;
    }

    public void setStfs(String stfs) {
        this.stfs = stfs == null ? null : stfs.trim();
    }

    public String getJh() {
        return jh;
    }

    public void setJh(String jh) {
        this.jh = jh == null ? null : jh.trim();
    }

    public String getYs() {
        return ys;
    }

    public void setYs(String ys) {
        this.ys = ys == null ? null : ys.trim();
    }

    public String getWh() {
        return wh;
    }

    public void setWh(String wh) {
        this.wh = wh == null ? null : wh.trim();
    }

    public String getTm() {
        return tm;
    }

    public void setTm(String tm) {
        this.tm = tm == null ? null : tm.trim();
    }

    public String getStms() {
        return stms;
    }

    public void setStms(String stms) {
        this.stms = stms == null ? null : stms.trim();
    }

    public String getLbbh() {
        return lbbh;
    }

    public void setLbbh(String lbbh) {
        this.lbbh = lbbh == null ? null : lbbh.trim();
    }

    public String getNgbm() {
        return ngbm;
    }

    public void setNgbm(String ngbm) {
        this.ngbm = ngbm == null ? null : ngbm.trim();
    }

    public String getWjxcsj() {
        return wjxcsj;
    }

    public void setWjxcsj(String wjxcsj) {
        this.wjxcsj = wjxcsj == null ? null : wjxcsj.trim();
    }

    public String getStflh() {
        return stflh;
    }

    public void setStflh(String stflh) {
        this.stflh = stflh == null ? null : stflh.trim();
    }

    public String getGdrq() {
        return gdrq;
    }

    public void setGdrq(String gdrq) {
        this.gdrq = gdrq == null ? null : gdrq.trim();
    }

    public String getSsbm() {
        return ssbm;
    }

    public void setSsbm(String ssbm) {
        this.ssbm = ssbm == null ? null : ssbm.trim();
    }

    public String getWjsx() {
        return wjsx;
    }

    public void setWjsx(String wjsx) {
        this.wjsx = wjsx == null ? null : wjsx.trim();
    }

    public String getLrr() {
        return lrr;
    }

    public void setLrr(String lrr) {
        this.lrr = lrr == null ? null : lrr.trim();
    }

    public String getWjlx() {
        return wjlx;
    }

    public void setWjlx(String wjlx) {
        this.wjlx = wjlx == null ? null : wjlx.trim();
    }

    public String getYjr() {
        return yjr;
    }

    public void setYjr(String yjr) {
        this.yjr = yjr == null ? null : yjr.trim();
    }

    public String getDjh() {
        return djh;
    }

    public void setDjh(String djh) {
        this.djh = djh == null ? null : djh.trim();
    }

    public String getSysFileCount() {
        return sysFileCount;
    }

    public void setSysFileCount(String sysFileCount) {
        this.sysFileCount = sysFileCount == null ? null : sysFileCount.trim();
    }

    public String getSwlx() {
        return swlx;
    }

    public void setSwlx(String swlx) {
        this.swlx = swlx == null ? null : swlx.trim();
    }

    public String getSwsm() {
        return swsm;
    }

    public void setSwsm(String swsm) {
        this.swsm = swsm == null ? null : swsm.trim();
    }

    public String getXcsj() {
        return xcsj;
    }

    public void setXcsj(String xcsj) {
        this.xcsj = xcsj == null ? null : xcsj.trim();
    }

    public String getFs() {
        return fs;
    }

    public void setFs(String fs) {
        this.fs = fs == null ? null : fs.trim();
    }

    public String getKh() {
        return kh;
    }

    public void setKh(String kh) {
        this.kh = kh == null ? null : kh.trim();
    }

    public String getAjh() {
        return ajh;
    }

    public void setAjh(String ajh) {
        this.ajh = ajh == null ? null : ajh.trim();
    }

    public String getSl() {
        return sl;
    }

    public void setSl(String sl) {
        this.sl = sl == null ? null : sl.trim();
    }

    public String getDocumentType() {
        return documentType;
    }

    public void setDocumentType(String documentType) {
        this.documentType = documentType == null ? null : documentType.trim();
    }
}