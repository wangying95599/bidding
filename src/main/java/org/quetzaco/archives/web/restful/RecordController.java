...
public class RecordController extends BaseRestContoller{
    ...
    public HttpEntity<APIEntity> insertDocumentToRecord(@AccessKey @PathVariable("recordId") Long recordId, @PathVariable("type") int type,
                                                        @RequestBody List<Documents> docs) {

        return buildEntity(APIEntity.create(recordService.insertDocumentToRecord(recordId, docs, type)), HttpStatus.ACCEPTED);
    }
    ...
    public HttpEntity<APIEntity<PageInfo>> searchGlobalRecordList(
            ...
            @RequestParam(value = "pageSize", required = false, defaultValue = "10000") int limit,HttpServletRequest request) {
