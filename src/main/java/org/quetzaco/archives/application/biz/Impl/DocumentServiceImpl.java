...
//      List<Documents> list = documentMapper.searchGlobalDocumentList(document);
	      ...
	      list = esDao.createSearch(document, list);
...
//    	System.setProperty("root","D:\\opt\\quetzaco\\quetapp");
    	String content = fileUtils.getFileContent(path);
    	System.out.println("content       "+content);
    	file.setContent(content);
    	try {
			esDao.deleteDocument("archive", "files", file.getFileId());
		} catch (Exception e) {
			e.printStackTrace();
		}
    	esDao.createIndex(file, "archive", "files");
    	return true;
    }
}
